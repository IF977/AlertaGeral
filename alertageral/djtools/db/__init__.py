    ...
    try:
        sql = """select 1 
                        ...
                        where a.attrelid = (select oid 
                                                    ...
                                                    where relname = '%s') 
                            and a.attname = '%s';""" % (table_name, column_name)
    ...
    try:
        sql = """select 1 
                        ...
                        where a.attrelid = (select oid 
                                                    ...
                                                    where relname = '%s') 
                            and a.attname = '%s' 
                            and pg_catalog.format_type(a.atttypid, a.atttypmod) = '%s';""" % (table_name, column_name, column_type)
        r = get_list(sql, conn=conn)[0]
        return bool(r)
    except:
        return False
...
def has_constrainttype(constraint_name,rel_table, conn=None):
    try:
        sql= """Select 1 FROM pg_constraint r
                    ...
                    Where conname = '%s' and X.relname = '%s';""" % (constraint_name, rel_table)
        r = get_list(sql, conn=conn)[0]
        return bool(r)
    except:
        return False
