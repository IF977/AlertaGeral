...
from djtools.formfields import DateTimeFieldPlus
from djtools.formwidgets import AutocompleteWidget
from djtools.templatetags.djtools_templatefilters import format_
...
#    )
    ...
    if form.non_field_errors():
        out.append(u'<ul class="errorlist">')
        for error in form.non_field_errors():
            out.append(u'<li>%s</li>' % error)
        out.append(u'</ul>')
        ...
        for line in lines:
            ...
            for field_name in line:
                ...
                if field.label:
                    ...
                    label = u' '.join(field_name.split('_')).capitalize()
                
                if isinstance(field, forms.BooleanField):
                    value = form.cleaned_data.get(field_name) and _(u'Yes') or _(u'No')
                else:
                    value = format_(form.cleaned_data[field_name]) or u'<em class="scarletred1">Não Informado</em>'
...
class FormPlus:
    ...
    class Media:
        css = {
            'all': ('/static/admin/css/forms.css',)
        }
    ...
    * Definição automática de widgets
    ...
    def get_field_attrs_for_modelchoicefield(self, fname):
        ...
        return dict(
            ...
            cache_choices = field.cache_choices,
            label         = field.label,
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(DtModelForm, self).__init__(*args, **kwargs)
        ...
        for fname, field in self.fields.items():
            ...
            if isinstance(field, forms.ModelChoiceField):
                new_f = forms.ModelChoiceField( 
                    **self.get_field_attrs_for_modelchoicefield(fname))
