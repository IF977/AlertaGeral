	...
	return{
		show:function(o){
			...
			if(!p){
				...
				g=document.createElement('div'); g.className='tclose'; g.v=0; g.innerHTML = '<a href="javascript:">fechar</a>';
...
function hideImg(){
	var img = document.getElementById('loading-tinybox');
	if(img!=null) img.parentNode.parentNode.removeChild(img.parentNode);
}

var loadJavaScript = true;
var URL = null;
function popup(url, width, height){
	showImg();
	if (loadJavaScript) {
		jQuery.getScript('/static/djtools/jquery/jquery.maskedinput.js');
		jQuery.getScript('/static/djtools/jquery/widgets-core.js');
		jQuery.getScript('/static/djtools/jquery/jquery.autocomplete.js');
		jQuery.getScript('/static/djtools/jquery/jquery.bgiframe.min.js');
		jQuery.getScript('/static/djtools/js/admin/RelatedObjectLookups.js');
		jQuery.getScript('/static/djtools/jstree/_lib.js');
		jQuery.getScript('/static/djtools/jstree/tree_component.min.js');
	...
	$.ajax({
		...
		success: function(data){
			...
			closejs = function(){
				...
				if(tokens.length>1) reload(tokens[1]);
				else reload('content');
				initAll();
			}
			var p = document.getElementById('feedback_message');
			if(p!=null) p.parentNode.removeChild(p);
			TINY.box.show({html:html,width:width,height:height,opacity:50,topsplit:3,openjs:openjs,closejs:closejs})
		}
	});
}
...
function setUpForms(){
	...
	for(var i=0; i<forms.length; i++){
		...
		form.onsubmit = function(){
			var params = 'popup=1&' + $(this).serialize();
			url = URL
			if(url.indexOf('?')==-1) url+='?popup=1'
			else url+='&popup=1'
			var xmlhttp = getActiveXObject();
			xmlhttp.open(form.method, url, true);
...
function replaceAddAnotherLinks(){
	...
	for(var i=0; i<links.length; i++){
		...
		if(link.href.indexOf('?_popup=1&source_input=')<0){
			link.href = link.href + '?_popup=1&source_input='+source_input;
		}
		link.onclick = function(){
			popup(this.href, 1000);
			return false;
		}
	}
	links = jQuery('.popup');
